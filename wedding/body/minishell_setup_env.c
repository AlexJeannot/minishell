...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/06/16 17:08:10 by ajeannot          #+#    #+#             */
/*   Updated: 2020/06/16 17:09:22 by ajeannot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	setup_oldpwd(void)
{
	...
	if ((oldpwd_index = search_in_array(global_env, "OLDPWD", '=')) == -1)
		global_env = extend_array_str(global_env, "OLDPWD",
		str_array_length(global_env));
}
...
void	setup_var(char *exe)
{
	...
	if (search_in_array(global_env, "_", '=') == -1)
	{
		...
		* (ft_strlen(pwd_path) + ft_strlen(exe) + 3));
		ft_strcpy(set_var, "_=");
		ft_strcat(set_var, pwd_path);
		ft_strcat(set_var, "/");
		ft_strcat(set_var, exe);
		global_env = extend_array_str(global_env, set_var,
		str_array_length(global_env));
	}
}
